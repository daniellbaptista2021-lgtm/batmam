...
def register_onboarding_routes(app) -> None:
    ...
    @app.post("/api/v1/onboarding/chatwoot/connect-existing", tags=["onboarding"])
    async def onboarding_chatwoot_connect_existing(request: _Req):
        """Modo B: cliente tem Chatwoot proprio, fornece admin API token.
        Clow testa conexao, descobre account_id, salva na base com is_remote=1.
        """
        sess = _auth(request)
        if not sess:
            return _JR({"error": "Nao autenticado"}, status_code=401)
        ...
        admin_token = (body.get("admin_api_token") or "").strip()
        account_id_input = body.get("account_id")

        if not chatwoot_url or not admin_token:
            return _JR({"error": "Informe URL e Admin API Access Token"}, status_code=400)
        if not chatwoot_url.startswith(("http://", "https://")):
            return _JR({"error": "URL invalida (deve comecar com https://)"}, status_code=400)

        # Testa: /api/v1/profile
        import urllib.request, json as _j
        try:
            req = urllib.request.Request(
                f"{chatwoot_url}/api/v1/profile",
                headers={"api_access_token": admin_token, "Content-Type": "application/json"},
            )
            with urllib.request.urlopen(req, timeout=15) as resp:
                profile = _j.loads(resp.read().decode())
        except Exception as e:
            return _JR({"error": f"Nao consegui conectar: {e}"}, status_code=400)

        # Descobre account_id (1o da lista ou o que user escolheu)
        accounts = profile.get("accounts") or profile.get("account") or []
        if not accounts and profile.get("account_id"):
            accounts = [{"id": profile["account_id"]}]
        if not accounts:
            return _JR({"error": "Token valido mas nenhuma conta associada"}, status_code=400)

        target_account_id = int(account_id_input) if account_id_input else int(accounts[0]["id"])
        # Valida que o token tem acesso a essa account
        valid_ids = [int(a.get("id", 0)) for a in accounts]
        if target_account_id not in valid_ids:
            return _JR({"error": f"Token nao tem acesso a conta {target_account_id}. Contas validas: {valid_ids}"}, status_code=400)

        # Salva
        from ..database import create_chatwoot_connection, get_chatwoot_connection_by_user, update_chatwoot_connection, get_db
        uid = _tenant(sess)
        existing = get_chatwoot_connection_by_user(uid)
        if existing:
            update_chatwoot_connection(uid,
                chatwoot_url=chatwoot_url,
                chatwoot_token=admin_token,
                chatwoot_account_id=target_account_id,
            )
        else:
        ...
        # Marca como remote + modo B
        try:
            with get_db() as db:
                db.execute(
                    "UPDATE chatwoot_connections SET is_remote=1, connection_mode=? WHERE user_id=?",
                    ("remote_managed", uid),
                )
                db.commit()
        except Exception:
            pass
        return _JR({"ok": True, "account_id": target_account_id, "chatwoot_url": chatwoot_url, "mode": "remote_managed"})


    @app.post("/api/v1/onboarding/chatwoot/connect-login", tags=["onboarding"])
    async def onboarding_chatwoot_connect_login(request: _Req):
        """Modo C: cliente so quer espelho (iframe). Fornece URL + email + senha.
        Clow testa login, salva URL+account_id, iframe aponta direto.
        Nao salva senha em claro — salva access_token obtido no sign_in.
        """
        sess = _auth(request)
        if not sess:
            return _JR({"error": "Nao autenticado"}, status_code=401)
        ...
        if not chatwoot_url or not email or not password:
            return _JR({"error": "Informe URL, email e senha"}, status_code=400)
        if not chatwoot_url.startswith(("http://", "https://")):
            return _JR({"error": "URL invalida (deve comecar com https://)"}, status_code=400)

        # Login via /auth/sign_in
        import urllib.request, json as _j
        try:
            payload = _j.dumps({"email": email, "password": password}).encode()
            req = urllib.request.Request(
            ...
            return _JR({"error": f"Login falhou: {e.code} {e.reason}"}, status_code=400)
        except Exception as e:
            return _JR({"error": f"Erro de conexao: {e}"}, status_code=400)
        ...
        if not accounts:
            return _JR({"error": "Login ok mas nenhuma conta encontrada"}, status_code=400)

        target_account_id = int(body.get("account_id") or accounts[0]["id"])
        ...
        try:
            with get_db() as db:
                db.execute(
                    "UPDATE chatwoot_connections SET is_remote=1, connection_mode=?, chatwoot_user_token=? WHERE user_id=?",
                    ("remote_only", access_token, uid),
                )
                db.commit()
        except Exception:
            pass
        return _JR({"ok": True, "account_id": target_account_id, "chatwoot_url": chatwoot_url, "mode": "remote_only"})
