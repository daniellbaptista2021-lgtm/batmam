...
async function del(id,email){if(!confirm(`Apagar definitivamente ${email}?

Isso remove o usuario, sua conta no Chatwoot e todos os dados relacionados (instancias WhatsApp, sessoes, creditos). Acao IRREVERSIVEL.`))return;const r=await fetch(`/api/v1/admin/users/${id}`,{method:'DELETE'});const d=await r.json().catch(()=>({}));if(r.ok&&d.ok){alert(`Apagado. Tabelas limpas: ${(d.tables_cleaned||[]).length}. Chatwoot account ${d.chatwoot_account_deleted?'removida':'nao removida'}.`);load();}else{alert('Erro: '+(d.error||r.status));}}
