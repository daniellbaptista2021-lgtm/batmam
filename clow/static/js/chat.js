...
function sendMessage(){const t=I.value.trim();if(!t&&!window._pf){if(proc)return;return}if(proc)return;if(http){sendHTTP(t||'Analise este arquivo');return}if(!ws||ws.readyState!==1)return;var msg={type:'message',content:t||'Analise este arquivo',model:selMod,conversation_id:cid};if(window._pf)msg.file_data=window._pf;addUser(t||'[arquivo enviado]');ws.send(JSON.stringify(msg));I.value='';I.style.height='auto';window._pf=null;document.getElementById('filePreview').innerHTML='';proc=true;SB.disabled=true;document.getElementById('stopBtn').classList.add('vis')}
async function sendHTTP(t){addUser(t);I.value='';I.style.height='auto';proc=true;SB.disabled=true;showThink();try{const r=await fetch('/api/v1/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.assign({content:t,session_id:hSid,conversation_id:cid,model:selMod},window._pf?{file_data:window._pf}:{}))});hideThink();if(!r.ok){const e=await r.json().catch(()=>({error:'Erro'}));showErr(e.error||e.response||'Erro');finishTurn();return}const d=await r.json();hSid=d.session_id||hSid;if(d.tools&&d.tools.length)d.tools.forEach(x=>{showTool(x.name,x.args);showToolR(x.name,x.status,x.output||'')});if(d.response){appendTxt(d.response);finishTxt()}if(d.file)showFile(d.file);if(d.mission)startPoll(d.mission);window._pf=null;document.getElementById('filePreview').innerHTML='';finishTurn()}catch(e){hideThink();showErr('Erro: '+e.message);finishTurn()}}
...
function finishTxt(){if(curBody){const c=curBody.querySelector('.scur');if(c)c.remove();if(raw&&typeof marked!=='undefined'){marked.setOptions({breaks:true,gfm:true});curBody.innerHTML=marked.parse(raw);curBody.querySelectorAll('.tline').forEach(function(el){el.innerHTML=linkify(el.innerHTML)});curBody.querySelectorAll('a').forEach(a=>{a.target='_blank';a.rel='noopener'})}raw=''}}
...
function linkify(t){return t.replace(/(https?:\/\/[^\s<]+)/g,'<a href="$1" target="_blank" rel="noopener" style="color:#7c5cfc;text-decoration:underline">$1</a>')}
let mPolls={};
...
function handleFileSelect(inp){
  ...
  var tm={xlsx:'spreadsheet',xls:'spreadsheet',csv:'spreadsheet',docx:'document',doc:'document',txt:'document',md:'document',py:'code',js:'code',ts:'code',html:'code',css:'code',json:'code',pdf:'pdf'};
  var tp=tm[ext]||'file';
  var rd=new FileReader();
  rd.onload=function(e){
    window._pf={type:tp,file_name:f.name,extracted_text:e.target.result,language:ext};
    document.getElementById('filePreview').innerHTML='<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:rgba(155,89,252,.1);border:1px solid rgba(155,89,252,.3);border-radius:8px;margin-bottom:4px;font-size:.82rem"><span style="flex:1">['+tp+'] '+f.name+'</span><button onclick="window._pf=null;this.parentElement.remove()" style="background:none;border:none;color:#f66;cursor:pointer">x</button></div>';
  };
  rd.readAsText(f);
  inp.value='';
}
// -- Drag & Drop --
(function(){
  var ov=document.getElementById('dragOv'),dc=0;
  document.addEventListener('dragenter',function(e){e.preventDefault();dc++;ov.style.display='flex'});
  document.addEventListener('dragleave',function(e){e.preventDefault();dc--;if(dc<=0){dc=0;ov.style.display='none'}});
  document.addEventListener('dragover',function(e){e.preventDefault()});
  document.addEventListener('drop',function(e){e.preventDefault();dc=0;ov.style.display='none';var f=e.dataTransfer.files[0];if(f){var dt=new DataTransfer();dt.items.add(f);var fi=document.getElementById('fileInp');fi.files=dt.files;handleFileSelect(fi)}});
})();
