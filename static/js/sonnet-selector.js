...
(function() {
  ...
  async function selectAgent(agent) {
    ...
    if (agent === 'sonnet') {
      ...
      // agent === 'clow' — back to DeepSeek chat
      if (currentAgent !== 'clow') {
        const confirm = await customConfirm(
          'Voltar ao Clow',
          'Uma nova conversa será iniciada. Deseja continuar?'
        );
        if (!confirm) return;
      }
      ...
      // Create new Clow conversation
      if (typeof window.newConv === 'function') {
        window.newConv();
      }
    }
  }
  ...
  function openSonnetIframe() {
    ...
    if (!wrap) {
      ...
      wrap.innerHTML = `
        ...
        <iframe id="sonnetIframe" src="${SYSTEM_CLOW_URL}" allow="clipboard-read; clipboard-write; microphone"></iframe>
      `;
  ...
  function init() {
    ...
    handleStripeReturn();
    // Restore selection if was on Sonnet
    if (currentAgent === 'sonnet') {
      fetchSonnetBalance().then(b => {
        if (b && b.has_credit && b.daily.remaining > 0 && b.weekly.remaining > 0) {
          openSonnetIframe();
        } else {
          currentAgent = 'clow';
          localStorage.setItem(STORAGE_KEY, 'clow');
